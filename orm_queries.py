...
#######    # 1. CREATE
    ...
    SubTask.objects.create(task=task, title="Gather information",
                           description="Find info", status="New",
                           deadline=today + timedelta(days=2))

    SubTask.objects.create(task=task, title="Create slides",
                           description="Create slides", status="New",
...
#####    # 2. READ
    ...
    print("2. New tasks:", list(Task.objects.filter(status="New")))
    print("   Expired Done subtasks:", list(SubTask.objects.filter(status="Done", deadline__lt=today)))
